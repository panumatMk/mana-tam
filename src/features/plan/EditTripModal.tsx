...
export const EditTripModal: React.FC<Props> = ({isOpen, onClose, onSave, initialTrip}) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!startDate || startDate === 'TBD') {
            ...
            errors.startDate = "วันเริ่มทริปต้องไม่ใช่อดีตนะ";
            hasError = true;
        }
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="ตั้งค่าทริป ✈️" footer={footerContent}>
            <form id="edit-trip-form" onSubmit={handleSubmit} className="space-y-6 py-2">
                ...
                <div>
                    <label
                        className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase tracking-widest">ชื่อทริป <span
                    ...
                    />
                    {formErrors.title &&
                        <div className="flex items-center gap-1 mt-2 text-red-500 text-[10px] font-bold animate-pulse">
                            <AlertCircle className="w-3 h-3"/>{formErrors.title}</div>}
                </div>

                {/* Date Inputs */}
                <div>
                    <label
                        className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase tracking-widest">วันไป <span
                        className="text-red-500">*</span></label>
                    <input
                        name="startDate"
                        type="date"
                    ...
                    {formErrors.startDate && <div
                        className="text-red-500 text-[10px] font-bold mt-1 leading-tight">{formErrors.startDate}</div>}
                </div>
                <div>
                    <label
                        className="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase tracking-widest">วันกลับ <span
                        className="text-red-500">*</span></label>
                    <input
                        name="endDate"
                        type="date"
                    ...
                    {formErrors.endDate && <div
                        className="text-red-500 text-[10px] font-bold mt-1 leading-tight">{formErrors.endDate}</div>}
                </div>
            </form>
        </Modal>
    );
